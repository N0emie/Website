// Advanced Loading Screen Management
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Advanced Loading Screen Started');
    ...
    // Ensure loading class is added immediately
    ...
    if (loadingVideoElement) {
        loadingVideoElement.addEventListener('error', function(e) {
            console.log('Loading video error, using fallback image');
    ...
    // Collect only CRITICAL resources for fast loading
    function collectCriticalResources() {
        const criticalResources = [
            // Essential UI elements
            'assets/webp/69.webp', // Main logo
            'assets/webp/telegram.webp', // Social icons
            'assets/webp/youtube.webp',
            'assets/webp/twitch.webp',
            // Loading screen video
            'assets/video/new_loading_logo.mp4',
            // Hero section essentials
            'assets/webp/okno.webp', // Window frames
    ...
    // Start loading process with smooth progress animation
    function startLoading() {
        const allResources = collectCriticalResources();
        totalResources = allResources.length;
        
        console.log(`📊 Found ${totalResources} resources to load`);
        
        // Start smooth progress animation from 0 to 100%
        startSmoothProgressAnimation();
        
        if (totalResources === 0) {
            console.log('⚠️ No resources found, will complete after animation');
            return;
        }
        
        // Load each resource in background (but don't rely on them for progress)
        allResources.forEach((resource, index) => {
            loadResource(resource, index);
        });
    }
    
    // Smooth progress animation that takes 3.5 seconds to reach 100%
    function startSmoothProgressAnimation() {
        ...
        function animateProgress() {
            ...
            console.log(`🎯 Progress animation: ${Math.floor(progress)}%`);
            
            if (progress < 100) {
                requestAnimationFrame(animateProgress);
            } else {
                // Animation complete, wait a bit then hide loading screen
                setTimeout(() => {
                    completeLoading();
                }, 500);
            }
    ...
    function loadVideo(url, index) {
        ...
        const onLoad = () => {
            if (!loadedResources.has(url)) {
                ...
                resourcesLoaded++;
                console.log(`✅ Video ${index + 1}/${totalResources} loaded: ${url.split('/').pop()}`);
            }
        };
        ...
        const onError = () => {
            if (!loadedResources.has(url)) {
                loadedResources.add(url);
                resourcesLoaded++;
                console.log(`❌ Video ${index + 1}/${totalResources} failed: ${url.split('/').pop()}`);
            }
        };
        ...
        setTimeout(() => {
            if (!loadedResources.has(url)) {
                console.log(`⏰ Video timeout: ${url.split('/').pop()}`);
                onError();
            }
        }, 10000);
    ...
    function loadImage(url, index) {
        ...
        const onLoad = () => {
            if (!loadedResources.has(url)) {
                loadedResources.add(url);
                resourcesLoaded++;
                console.log(`✅ Image ${index + 1}/${totalResources} loaded: ${url.split('/').pop()}`);
            }
        };
        ...
        const onError = () => {
            if (!loadedResources.has(url)) {
                loadedResources.add(url);
                resourcesLoaded++;
                console.log(`❌ Image ${index + 1}/${totalResources} failed: ${url.split('/').pop()}`);
            }
        };
        ...
        setTimeout(() => {
            if (!loadedResources.has(url)) {
                console.log(`⏰ Image timeout: ${url.split('/').pop()}`);
                onError();
            }
        }, 8000);
    ...
    function completeLoading() {
        ...
        // Ensure minimum loading time of 4 seconds for smooth progress animation
        const minLoadingTime = 4000;
        const startTime = Date.now();
        const elapsedTime = startTime - window.loadingStartTime || 0;
        ...
        setTimeout(() => {
            ...
            const hiddenElements = document.querySelectorAll('.social-icons-corner, .hero-logo-corner, .hero-lisa, .main-content, .examp-background, .examp-fog-left, .examp-fog-right, .examp-crystal, .examp-dota-logo, .examp-gif');
    ...
    // Safety fallback - ensure loading completes after 6 seconds maximum
    setTimeout(() => {
        if (body.classList.contains('loading')) {
            console.log('🚨 Safety fallback triggered - forcing completion');
            completeLoading();
        }
    }, 6000);
    ...
    // Setup lazy loading for videos after main loading completes
    ...
    // Setup lazy loading after page loads
    window.addEventListener('load', setupLazyVideoLoading);
});
