// Функция создания анимированных точек для экрана загрузки
function createLoadingFloatingDots() {
    const container = document.getElementById('loadingFloatingDots');
    if (!container) return;
    
    // Создаем 30 точек
    for (let i = 0; i < 30; i++) {
        const dot = document.createElement('div');
        dot.className = 'loading-floating-dot';
...
document.addEventListener('DOMContentLoaded', function() {
 console.log('🚀 Starting smart loading system with fixed progress...');
 
 // Создаем анимированные точки
 createLoadingFloatingDots();
 ...
 console.log('🚀 Starting fixed progress bar (0-100% in 2.5s)');
 ...
 console.log(`📊 Tracking ${criticalImages.length} critical images in background`);
 let loadedImages = 0;
 const totalImages = criticalImages.length;
 const checkAllLoaded = () => {
 loadedImages++;
 console.log(`✅ Critical image loaded (${loadedImages}/${totalImages})`);
 if (loadedImages >= totalImages) {
 this.criticalContentReady = true;
 console.log('🎉 All critical content ready!');
 }
 };
 ...
 this.criticalContentReady = true;
 console.log('⏰ Critical content marked as ready (timeout)');
 }
 }, 2000);
 }
 completeLoading() {
 ...
 console.log(`🎉 Fixed progress complete! Total time: ${elapsed}ms`);
 console.log(`📊 Critical content ready: ${this.criticalContentReady}`);
 setTimeout(() => {
 hideLoadingScreen();
 ...
 console.log('🎬 Starting lazy loading of videos...');
 const videos = document.querySelectorAll('video');
 videos.forEach((video, index) => {
 const observer = new IntersectionObserver((entries) => {
 entries.forEach(entry => {
 if (entry.isIntersecting) {
 console.log(`🎬 Video ${index + 1} is visible, starting load...`);
 ...
 console.log(`⏱️ Adjusted loading duration: ${this.loadingDuration}ms`);
 }
 }
 }
